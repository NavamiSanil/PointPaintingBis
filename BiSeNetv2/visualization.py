...
class KittiVisualizer:
    ...
    def add_semantic_to_image(self, image, semantic):
        return cv2.addWeighted(cv2.resize(image, (semantic.shape[1], semantic.shape[0])).astype(np.uint8), 1,
                               semantic.astype(np.uint8), .5, 0, cv2.CV_32F)
    ...
    def visualize_horizontal(self, image, semantic):
        ...
        self.__save(total_image, 'horizontal_visualization.png')
    ...
    def __visualize(self, image, semantic, label):
        ...
        self.__save(total_image, 'visualization_output.png')
...
def main():
    ...
    for i in range(len(dataset)):
        ...
        visualizer.__save(new_img, f'visualized_image_{i}.png')  # Save each image with a unique name
